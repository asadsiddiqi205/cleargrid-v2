...
const outcomeConfig: Record<string, { icon: typeof Square; color: string; bg: string }> = {
  ...
  Converted: { icon: CheckCircle, color: "text-emerald-400", bg: "bg-emerald-500/15" },
  Exhausted: { icon: AlertTriangle, color: "text-amber-400", bg: "bg-amber-500/15" },
  Unresponsive: { icon: XCircle, color: "text-red-400", bg: "bg-red-500/15" },
  Exited: { icon: Power, color: "text-red-400", bg: "bg-red-500/15" },
};
...
export function EndNode({ data, selected }: NodeProps) {
  ...
  const config = outcomeConfig[outcome ?? ""] ?? {
    icon: MinusCircle, color: "text-red-400", bg: "bg-red-500/15",
  };
  ...
  return (
    <div
      className={cn(
        ...
        selected
          ? "border-red-500/60 shadow-lg shadow-red-500/20 ring-2 ring-red-500/40"
    ...
    >
      ...
      <div className="flex items-center gap-2.5 bg-red-500/10 px-3 py-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[9px] uppercase tracking-wider text-red-400/80">
            Exit / Governance
          </div>
        </div>
      </div>
      ...
      {typeof simCount === "number" && (
        <div className="absolute -right-2 -top-2 z-10 rounded-full bg-red-500 px-2 py-0.5 text-[10px] font-semibold text-red-950 shadow-lg">
